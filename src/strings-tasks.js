...
function getFirstChar(value) {
  const res = !value ? '' : value.charAt(0);
  return res;
  // throw new Error('Not implemented');
}
...
function repeatString(str, times) {
  const res = times < 0 ? '' : str.repeat(times);
  return res;
  // throw new Error('Not implemented');
}
...
function countVowels(str) {
  const len = str.match(/[aeiouAEIOUY]/gi);
  const res = len ? len.length : 0;
  return res;
  // throw new Error('Not implemented');
}
...
function reverseWords(str) {
  const s = str.split(' ');
  const m = s.map((word) => word.split('').reverse().join(''));
  const res = m.join(' ');
  return res;
  // throw new Error('Not implemented');
}
